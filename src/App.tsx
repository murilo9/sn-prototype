...
function App() {
  ...
  const [weekDays, setWeekDays] = useState<{ [key: string]: TimeSpan[] }>({
    ...weekDaysMock,
  });
  ...
  const getMinutesString = (minutesNumber: number) =>
    minutesNumber > 9
      ? minutesNumber.toString()
      : "0" + minutesNumber.toString();

  const detectConflictBetweenRanges = (
    startTime1: number,
    endTime1: number
  ) => {
    ...
    dayTimeSpans.forEach((timeSpan) => {
      startTime2 = Number(
        timeSpan.start.hours.toString() +
          getMinutesString(timeSpan.start.minutes)
      );
      endTime2 = Number(
        timeSpan.end.hours.toString() + getMinutesString(timeSpan.end.minutes)
      );
  ...
  const validateTimespan = (
    rawFromTimeNumericValue: number,
    rawToTimeNumericValue: number
  ) => {
    ...
    if (
      rawToTimeNumericValue - rawFromTimeNumericValue <
      MINIMUM_TIMESPAN_MINS
    ) {
      setError("Time spans must be at least 15 min long.");
      return false;
    }
    if (
      detectConflictBetweenRanges(
        rawFromTimeNumericValue,
        rawToTimeNumericValue
      )
    ) {
  ...
  const deleteTimespan = (id: string, weekDay: string) => {
    const updatedWeekDays = { ...weekDays };
    const index = updatedWeekDays[weekDay].findIndex(
      (timespan) => timespan.id === id
    );
    if (index >= 0) {
  ...
  return (
    <div className="App">
      <Box sx={{ px: 2 }}>
        <FormControlLabel
          control={<Switch onChange={() => setIsOwner(!isOwner)} />}
          label={
            isOwner ? "My own availiability" : "Someone else's availability"
          }
        />

        <Paper
          sx={{ maxWidth: "768px", margin: "48px auto", py: 2, pl: 2, pr: 6 }}
        >
          <Box
            sx={{
              marginLeft: "120px",
          ...
          <CardContent>
            ...
            >
              <Box>
                ...
                <Typography variant="caption">Someone Else</Typography>
              </Box>
              <IconButton>
                <Close />
              </IconButton>
            ...
            <TextField
              ...
              rows={4}
              fullWidth
              label="Ask something about the day"
              autoFocus
              sx={{ mt: 2 }}
            />
          </CardContent>
      ...
      <Dialog
        onClose={() => setShowAddTimespanDialog(false)}
        open={showAddTimespanDialog}
      >
        <Card sx={{ minWidth: "340px" }}>
          <CardContent>
            ...
            >
              <Box>
                <Typography variant="h6">Add time span</Typography>
                <Typography variant="caption">{addTimespanWeekday}</Typography>
              </Box>
          ...
          <CardActions sx={{ justifyContent: "flex-end" }}>
            <Button
              color="error"
              onClick={() => setShowAddTimespanDialog(false)}
            >
              Cancel
            </Button>
